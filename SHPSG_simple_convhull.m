...
ix = linspace(Xlim(1),Xlim(2),dim);
iy = linspace(Ylim(1),Ylim(2),dim);
iz = linspace(Zlim(1),Zlim(2),dim);
...
% check if points are in 3D convex object
...
attenCoeff=175;
 ...
 % Do projection:
    ImgBinary = img;
    ImgBinary(img>0)=1;
    flattened = -sum(ImgBinary,2);
    projection = peakValues*exp(squeeze(flattened)/attenCoeff);
    figure; imshow(projection,[10000,65355]);
